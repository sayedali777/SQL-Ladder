...
select name from pets where species = 'dog' and age < 5;

-- 11) Query all the pets in the `pets` table that are either male dogs or female cats.

select * from pets where (species = 'dog' and sex ='M') or (species='cat' and sex = 'F');
...
select name, age from pets where species = 'cat' and sex = 'F' order by age DESC;

-- 14) Get all pets from `pets` whose names start with P.

select * from pets where lower(name) like 'p%';
...
select * from employees where  job <> 'Sales' and (lower(firstname) = 'robert' or lower(firstname) = 'lisa' or lower(firstname) like 'j%' );
...
select firstname, lastname,job, round(cast(salary as NUMERIC) / 1.1,2) from employees;

-- 24) Repeat the previous problem, but rename the column `salary_eu`.

select firstname, lastname,job, round(cast(salary as NUMERIC) / 1.1,2) as salary_eu from employees;

-- 25) Query the `employees` table, but combine the `firstname` and `lastname` columns to
-- be "Firstname, Lastname" format. Call this column `fullname`. For example,
-- the first row should contain `Thompson, Christine` as `fullname`. Also, display the rounded `salary_eu` instead of `salary`.
--     * _Hint:_ The string concatenation operator is `||`

select lastname || ', ' || firstname as fullname, job, round(cast(salary as NUMERIC) / 1.1,2) as salary_eu from employees;
...
select lastname || ', ' || firstname as fullname, job, round(cast(salary as NUMERIC) / 1.1,2) as salary_eu, substr(startdate,1,4) as startyear from employees;


-- 27) Repeat the above problem, but instead of using `SUBSTR()`, use `STRFTIME()`.

select lastname || ', ' || firstname as fullname, job, round(cast(salary as NUMERIC) / 1.1,2) as salary_eu, strftime('%Y', startdate) as startyear from employees;

-- 28) Query the `employees` table, replacing `firstname`/`lastname` with `fullname` and `startdate` with `startyear`. 
--Print out the salary in USD again, except format it with a dollar sign, comma separators, and no decimal. For example, the first row should read `$123,696`.
-- This column should still be named `salary`.
...
select lastname || ', ' || firstname as fullname, job, substr(startdate,1,4) as startyear, printf('$%,.2d', salary) as salary from employees;
-- 
...
select *, case when  job = 'Sales' then 'Yes'
else 'No'
end as bonus 
from employees;
-- 30) This year, only sales people with a salary of $100,000 or higher are eligible for bonuses. Create a `bonus` 
--column like in the last problem for salespeople with salaries at least $100,000.

select *, case when  job = 'Sales' and salary >= 100000 then 'Yes'
else 'No'
end as bonus
from employees;

-- 31) Next year, the bonus structure will be a little more complicated. You'll create a `target_comp` 
--column which represents an employee's target total compensation after their bonus. Here is the company's bonus structure:
-- 
